...
MatchSchema.path('constraintKey').default(function(){
  return this.host.MatchInfo.Section+visitor.MatchInfo.classVisitNumber;
});
...
Match.prototype.toClientObject = function() {
      ...
      returnObject.matchCount = match.count;
      //this loops through the keys and determines what we are matched on
      for(var i=0;i<match.keys.length;i++) {
        returnObject[match.keys[i]]=match.matchedOn[match.keys[i]];
      }
      return returnObject;
    };
